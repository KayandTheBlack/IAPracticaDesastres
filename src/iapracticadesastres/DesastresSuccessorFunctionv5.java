 ...
 *
 */
public class DesastresSuccessorFunctionv5 implements SuccessorFunction { // This version checks all possible values
    @SuppressWarnings("unchecked")
    public List getSuccessors(Object aState) {
        ArrayList                retVal = new ArrayList();
        DesastresBoardv4             board  = (DesastresBoardv4) aState;
                    ...
                    for (int h2 = 0; h2 < travels.size(); h2++) {
                        for (int f2 = 0; f2 < travels.get(h2).size(); f2++) {
                            if(board.possibleMovAndDeletev2(h1,h2,f1,g1,f2)) {
                                DesastresBoardv4 board3 = board.clone();
